<div>
    <x-modal name="userEditModal{{ $user->id }}" :escapeClose="false" maxWidth="md" :show="$errors->isNotEmpty()" focusable>
        <div class="card bg-white rounded-lg shadow-md overflow-hidden">
            <form wire:submit.prevent="update" class="space-y-4">
                <div class="card-header p-4 border-b border-gray-200 flex justify-between">
                    <h2 class="text-lg font-medium text-gray-900">
                        Kemaskini Pengguna
                    </h2>
                </div>
                <div class="card-body p-4">

                    <div class="mb-3">
                        <x-text-input :label="__('Name')" wire:model.blur="name" id="name" class="mt-1 block w-3/4"
                            placeholder="{{ __('Name') }}" />
                    </div>
                    <div class="mb-3">
                        <x-text-input :label="__('Email')" wire:model.blur="email" id="email" class="mt-1 block w-3/4"
                            placeholder="{{ __('Email') }}" />
                    </div>
                    <div class="mb-3 ">
                        <label class="block mb-1">Peranan</label>
                        <div class="flex">
                            @foreach ($roles as $key => $role)
                                <div class="flex items-center me-4">
                                    <input type="checkbox" id="checkbox-{{ $key }}" wire:model="role_ids"
                                        value="{{ $role->name }}"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600  focus:ring-2 ">
                                    <label for="checkbox-{{ $key }}"
                                        class="ms-2 text-sm font-medium text-gray-900">
                                        {{ $role->name }}
                                    </label>
                                </div>
                            @endforeach
                        </div>
                        <x-input-error :messages="$errors->get('tag_ids')" class="mt-2" />
                    </div>
                </div>

                <div class="card-footer p-4 bg-gray-100 border-t border-gray-200 mt-6 flex justify-end">
                    <x-secondary-button type="button" x-on:click="$dispatch('close');  $wire.clearModalValidation?.();"
                        class="ms-1">
                        <x-icons.cancel size="size-4" />
                        Batal
                    </x-secondary-button>
                    <x-success-button x-on:click="watingUpdateUser()" class="ms-1">
                        <x-icons.pencil size="size-4" />
                        Kemaskini
                    </x-success-button>
                </div>
            </form>
        </div>
    </x-modal>
</div>
